...
for (const [what, sh, ref] of [["combined factor book, modern era", 0.40, "D-527/558"], ["crypto GBM candidate, liquid tercile (unholdable: 3.7y underwater)", 1.11, "D-535/565"], ["hourly adaptive set-ups, 24 instruments", 0.0, "D-861 (gross zero)"], ["diversified TSMOM book, 110 assets, OOS 2015-26 (long basket did 0.80)", 0.62, "D-863"]] as [string, number, string][]) console.log(`    ${what.padEnd(64)} Sharpe ${isNaN(sh) ? " pending" : sh.toFixed(2).padStart(8)}   ${ref}`);
