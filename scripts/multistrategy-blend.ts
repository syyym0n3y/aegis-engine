...
const IV_FILE = Deno.env.get("IV_SPEC") || "d939-ivol-daily.json"; // D-953: override to A/B a concentrated (hand-fillable) ivol series; deployed d939 stays default
if (_SLiv()) { const iv = JSON.parse(await Deno.readTextFile(new URL(`../data/${IV_FILE}`, import.meta.url))) as { d: string; ret: number }[]; OS.ivol = new Map(iv.filter((x) => isOOS(x.d)).map((x) => [x.d, x.ret])); if (IV_FILE !== "d939-ivol-daily.json") console.log(`  [D-953 A/B] ivol sleeve from ${IV_FILE} (NOT deployed d939)`); }
...
// D-953b: DEPLOYED squeeze de-gross (the frozen D-947 rule applied to the LIVE book). Cut (1-f) of the distress+ivol
// contribution to cash on any day the trailing-K short-book bleed is below the frozen threshold (through i-1, LAG-1, no
// look-ahead). Squeeze INSURANCE: ~neutral Sharpe in calm windows, protects the acute tail. Modifies blend[] in place so
// every downstream stat / the PAPER mark reflect the de-grossed book. blendNet is unchanged (distress+ivol are dollar-neutral).
if (K.DEGROSS_LIVE === "1") { const iGc = names.indexOf("gcshort"), iIv = names.indexOf("ivol");
  if (iGc >= 0 && iIv >= 0) { const Kk = +K.DEGROSS_K, tau = +K.DEGROSS_TAU, f = +K.DEGROSS_F; const shortBook = days.map((_, i) => (X.gcshort[i] + X.ivol[i]) / 2); let nCut = 0;
    for (let i = 0; i < days.length; i++) { let bleed = 0; for (let j = Math.max(0, i - Kk); j < i; j++) bleed += shortBook[j]; if (bleed < tau) { blend[i] = blend[i] - (1 - f) * (blendW[i][iGc] * X.gcshort[i] + blendW[i][iIv] * X.ivol[i]); nCut++; } }
    console.log(`  [D-953b DE-GROSS LIVE] frozen: cut ${((1 - f) * 100).toFixed(0)}% of distress+ivol when the ${Kk}d short-book bleed < ${(100 * tau).toFixed(1)}% — fired ${nCut}/${days.length} days (${(100 * nCut / days.length).toFixed(1)}%)`); }
  else console.log(`  [D-953b DE-GROSS LIVE] needs gcshort+ivol in SLEEVES — not applied.`); }
...
// ---- SCALE THE PROVEN EDGE (read-only, default off) — D-950: turn the measured excess Sharpe into a wealth trajectory.
// The EMPIRICAL growth rate g(L) = 252*mean(log(1 + L*excess_daily)) accounts for FAT TAILS and drawdowns (the S^2/2
// formula assumes lognormal and OVERSTATES the safe leverage). Financing is net-exposure (D-913): excess_daily =
// (blend - rf*net) scaled to the vol target. Reports growth-optimal (full-Kelly) leverage, the standard fractional-Kelly
// sizes with their MEASURED maxDD + time-underwater, and the time to concrete wealth targets. Kelly is the CEILING, not
// the recommendation — full-Kelly drawdowns are ruinous to hold; half-to-quarter Kelly is the deployable range. ----
if (K.SCALE === "1") {
  const rfDay = (d: string) => { const r = usAt(d); return r === null ? 0 : (r / 100) / 252; };
  const lvBase = +K.VOL_TARGET / (bv || 1);                       // leverage that puts the raw book at the 10% vol target
  const exc = days.map((d, i) => (blend[i] - rfDay(d) * blendNet[i]) * lvBase); // the net-exposure-financed excess daily series AT 10% vol
  const rf = mean(days.map(rfDay)) * 252;
  // empirical geometric growth of $1 at leverage multiple m (m=1 => the 10% vol book); guards ruin (daily loss > 100%)
  const growth = (m: number) => { let g = 0, ruin = false; for (const e of exc) { const x = 1 + m * e; if (x <= 0) { ruin = true; break; } g += Math.log(x); } return ruin ? -Infinity : (g / exc.length) * 252 + rf; };
  const measured = (m: number) => { const s = stats(exc.map((e) => e * m)); return { vol: s.vol, mdd: s.mdd, uwY: s.uwY }; };
  // find the empirical growth-optimal m* (fine grid)
  let mStar = 0, gStar = -Infinity; for (let m = 0.1; m <= 12; m += 0.1) { const g = growth(m); if (g > gStar) { gStar = g; mStar = m; } }
  const volAt = (m: number) => +K.VOL_TARGET * m;                 // book vol at leverage multiple m
  console.log(`\n==> D-950 SCALE THE PROVEN EDGE — excess Sharpe ${(mean(exc) / (sd(exc) || 1) * Math.sqrt(252)).toFixed(2)}, rf ${(100 * rf).toFixed(1)}%, ${exc.length} days`);
  console.log(`  EMPIRICAL growth-optimal (full-Kelly) leverage m* ${mStar.toFixed(1)}x -> ${(100 * volAt(mStar)).toFixed(0)}% vol, growth ${(100 * gStar).toFixed(0)}%/yr, maxDD ${(100 * measured(mStar).mdd).toFixed(0)}% (RUINOUS to hold — the ceiling, not the size)`);
  console.log(`  ${"sizing".padEnd(16)} ${"lev".padStart(5)} ${"vol".padStart(6)} ${"growth/yr".padStart(10)} ${"maxDD".padStart(7)} ${"underwater".padStart(11)}`);
  const rows: [string, number][] = [["full-Kelly", mStar], ["half-Kelly", mStar / 2], ["quarter-Kelly", mStar / 4], ["fixed 10% vol", 1], ["fixed 20% vol", 2], ["fixed 40% vol", 4]];
  for (const [nm, m] of rows) { const g = growth(m), me = measured(m); console.log(`  ${nm.padEnd(16)} ${m.toFixed(1).padStart(4)}x ${(100 * volAt(m)).toFixed(0).padStart(5)}% ${(g > -Infinity ? (100 * g).toFixed(0) + "%" : "RUIN").padStart(10)} ${(100 * me.mdd).toFixed(0).padStart(6)}% ${me.uwY.toFixed(1).padStart(9)}y`); }
  // wealth trajectory at the deployable (half-Kelly) and a conservative (quarter-Kelly / 20% vol) size
  const S0 = +K.SCALE_START; const targets: [string, number][] = [["10x", 10], ["100x", 100], ["$1M", 1e6 / S0], ["$10M", 1e7 / S0]];
  const yearsTo = (g: number, mult: number) => g > 0 ? Math.log(mult) / g : Infinity;
  console.log(`\n  WEALTH TRAJECTORY from $${S0.toLocaleString()} (years to target; the drawdowns above are what you must HOLD through):`);
  console.log(`  ${"size".padEnd(16)} ${"growth".padStart(7)} ${targets.map(([t]) => t.padStart(9)).join("")}`);
  for (const [nm, m] of [["half-Kelly", mStar / 2], ["quarter-Kelly", mStar / 4], ["fixed 20% vol", 2]] as [string, number][]) { const g = growth(m);
    console.log(`  ${nm.padEnd(16)} ${(100 * g).toFixed(0).toString().padStart(6)}% ${targets.map(([, mult]) => { const y = yearsTo(g, mult); return (y === Infinity ? "never" : y < 100 ? y.toFixed(1) + "y" : ">100y").padStart(9); }).join("")}`); }
  console.log(`  NOTE: growth uses the net-exposure-financed EXCESS (D-913). Half-Kelly is the aggressive-but-holdable ceiling; quarter-Kelly/20% vol is the deployable size. The binding real-world constraints are BELOW.`);
  console.log(`  CAPACITY + MINIMUM CAPITAL (the real ceilings, honestly bounded not fake-precise):`);
  console.log(`   - CAPACITY: the distress+ivol legs are liquid-tercile long-shorts on ~700 (distress) / ~3000 (ivol) names; the binding limit is the DISTRESS SHORT (small/mid-cap borrow) — low single-digit $M before impact/borrow scarcity bites. Trend/long/crypto scale far higher. So the BOOK's edge is intact to ~$1-5M, decaying above.`);
  console.log(`   - MIN CAPITAL: the full name-level book needs ~$50-250k to hold 5 sleeves at whole-share/whole-lot granularity; an ETF-proxy implementation (sector/factor ETFs + micro futures) runs from ~$5-25k. Below that, granularity noise swamps the edge — $100 cannot hold this book (the D-948/949 intraday path was the only $100-native route, and it did not survive).`);
}
