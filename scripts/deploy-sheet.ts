...
console.log(`DISTRESS SHORT — ${distActive.size} names active; hold the ${distTop.length} MOST-LIQUID (fillable+borrowable, ~$${Math.round(perGc)}/name each):\n  ${distTop.join(" ")}${degrossed ? "   [FLAT while de-gross active]" : ""}`);
console.log(`  [!] distress is event-binary: the deployed sleeve shorts all ${distActive.size}; concentrating it to the top-${NPL} liquid (with measured edge retention, the IVOL precedent) is the next deploy step.`);
