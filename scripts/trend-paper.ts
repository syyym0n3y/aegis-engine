...
for (const d of [...new Set([...tr.keys(), ...cr.keys()])].sort()) { if (have.has(d) || d >= today) continue; const n = nAct.get(d) ?? 1; const t = (tr.get(d) ?? 0) / n, l = (lg.get(d) ?? 0) / n;
  ...
  const last = ledger[ledger.length - 1]; const h = ledger.slice(-61, -1); const va = h.length > 40 ? sd(h.map((x) => x.timedcpv ?? 0)) : 0, vb = h.length > 40 ? sd(h.map((x) => x.cryptotimed ?? 0)) : 0; const wa = va > 0 && vb > 0 ? (1 / va) / (1 / va + 1 / vb) : 0.5; last.pair = +(wa * (last.timedcpv ?? 0) + (1 - wa) * (last.cryptotimed ?? 0)).toFixed(8); added++; }
