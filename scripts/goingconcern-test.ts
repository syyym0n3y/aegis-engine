...
// D-936: dump raw panel-present going-concern events (ticker+date) for the combined distress-sleeve builder.
// This script no longer writes the daily sleeve itself — distress-sleeve.ts is the single owner of d931, fed by
// BOTH going-concern (here) and late-filing (latefiling-test.ts) events, with liquidity computed on the union.
const gcEvents = hits.filter((h) => px.has(h.ticker)).map((h) => ({ ticker: h.ticker, date: h.date }));
await Deno.writeTextFile(new URL("../data/d930-gc-events.json", import.meta.url), JSON.stringify(gcEvents));
console.log(`  dumped ${gcEvents.length} panel going-concern events (${new Set(gcEvents.map(e=>e.ticker)).size} names) -> data/d930-gc-events.json`);
