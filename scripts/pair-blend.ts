...
// per-year stability of the parity pair (descriptive): a book that lives in one year is not a book
{ const yrs = [...new Set(days.map((d) => d.slice(0, 4)))]; console.log(`  BY YEAR (parity, 10% vol): ` + yrs.map((y) => { const ix = days.map((d, i) => [d, i] as [string, number]).filter(([d]) => d.startsWith(y)).map(([, i]) => i); const x = ix.map((i) => blends.parity[i]); const s = stats(x, ANN); return `${y} ${(100 * s.mu).toFixed(0)}% (SR ${s.sr.toFixed(1)})`; }).join(" | ")); }
