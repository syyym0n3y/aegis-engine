...
// ftd-vs-shortinterest.ts (D-616) — the only MECHANICAL forced-flow event in any dataset we hold.
//
...
const K = declareKnobs("ftd-vs-shortinterest", [
  ...
  { name: "RANKBY", def: "ftd", note: "ftd = persistence (D-615) | si = short interest, the matched control" },
...
async function jwt() {
  ...
  const h = e({ alg: "HS256", typ: "JWT" }), b = e({ role: "service_role", iss: "fv", exp: 4102444800 });
...
// D-616: the matched control variable. Short interest is published twice monthly by FINRA; days_cover (short
// quantity / average daily volume) is the standard normalisation and is what the board's si_dtc specs already use.
const si = new Map<string, { d: string; dtc: number }[]>();
if (K.RANKBY === "si") {
  for (let off = 0;; off += 50000) {
    const rows = await fetch(`${OWNED}/trd_short_interest?select=symbol,settlement,days_cover&order=settlement&offset=${off}&limit=50000`, { headers: hdr })
      .then((r) => r.json()).catch(() => []) as { symbol: string; settlement: string; days_cover: number }[];
    if (!Array.isArray(rows) || !rows.length) break;
    for (const r of rows) {
      if (!priced.has(r.symbol)) continue;
      const v = +r.days_cover; if (!Number.isFinite(v) || v <= 0) continue;
      (si.get(r.symbol) ?? si.set(r.symbol, []).get(r.symbol)!).push({ d: r.settlement, dtc: v });
    }
    if (rows.length < 50000) break;
  }
  assertNonEmpty("symbols with short interest", [...si.keys()], 200);
  for (const a of si.values()) a.sort((x, y) => x.d < y.d ? -1 : 1);
}
const siAt = (sym: string, d: string): number | null => {
  const a = si.get(sym); if (!a) return null;
  let lo = 0, hi = a.length - 1, at = -1;
  while (lo <= hi) { const m = (lo + hi) >> 1; if (a[m].d <= d) { at = m; lo = m + 1; } else hi = m - 1; }
  return at >= 0 ? a[at].dtc : null;
};
...
for (const [, group] of [...byM.entries()].sort()) {
  ...
  for (const e of group) {
    ...
    // IDENTICAL event set, dates, holds and costs — only the ranking variable differs, which is what makes the
    // alpha regression a like-for-like control rather than a comparison of two different studies.
    let sig: number | null = e.runLen * e.failRatio;
    if (K.RANKBY === "si") sig = siAt(e.s, e.pubD);
    if (sig === null || !Number.isFinite(sig)) continue;
    cands.push({ sig, ret: r, dvol: k ? dv / k : 0 });
  }
...
if (K.DUMP) {
  const dates = [...byM.keys()].sort();
  await Deno.writeTextFile(K.DUMP, periodRets.map((r, i) => `${dates[i] ?? i}\t${r}`).join("\n"));
}

const m = mean(periodRets), s2 = sd(periodRets) || 1e-12;
const tPort = m / (s2 / Math.sqrt(periodRets.length));
console.log(`\n==> RANKED BY ${K.RANKBY.toUpperCase()} — measured from PUBLICATION (settle + ${LAG}d), not settle date`);
