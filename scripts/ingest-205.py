#!/usr/bin/env python3
# ingest-402.py (D-511) — 8-K Item 2.05 events via EDGAR full-text search, monthly windows, sequential.
...
while d<=dt.date(2026,8,1):
    ...
    frm=0
    while True:
        q=urllib.parse.urlencode({"q":'"Item 2.05"',"forms":"8-K","dateRange":"custom",
        ...
        for h in hits:
            s=h["_source"]
            if "2.05" not in (s.get("items") or []): continue
...
sql=("create temp table stage_e (accession text, symbol text, filed date);\n"
     ...
     "insert into trd_events_205 select * from stage_e on conflict (accession) do nothing;\n")
...
print(f"TOTAL {len(uniq):,} 2.05 events")
