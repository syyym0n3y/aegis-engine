...
wc -l cusip_map.tsv
$PSQL <<SQL
create temp table stage_map (cusip text, symbol text);
\copy stage_map from 'cusip_map.tsv' with (format text)
insert into trd_cusip_map select distinct on (cusip) cusip, symbol from stage_map
...
for q in 2019q4 2020q1 2020q2 2020q3 2020q4 2021q1 2021q2 2021q3 2021q4 2022q1 2022q2 2022q3 2022q4 \
         2023q1 2023q2 2023q3 2023q4 2024q1 2024q2 2024q3 2024q4 2025q1 2025q2 2025q3 2025q4 2026q1 2026q2; do
  z="${q}_nport.zip"
  if [ ! -s "$z" ]; then
  ...
  unzip -p "$z" FUND_REPORTED_HOLDING.tsv | awk -F'\t' -v SUB=sub.tsv '
    BEGIN{ split("JAN FEB MAR APR MAY JUN JUL AUG SEP OCT NOV DEC",mn," "); for(i=1;i<=12;i++) M[mn[i]]=sprintf("%02d",i);
      while((getline line < SUB)>0){ n=split(line,a,"\t"); if(a[1]=="ACCESSION_NUMBER") continue;
  ...
  $PSQL <<SQL
create temp table stage_np (cusip text, report_date date, n_positions int, shares float8, value_usd float8);
\copy stage_np from 'agg.tsv' with (format text)
insert into trd_nport_ownership (cusip, report_date, effective_date, n_positions, shares, value_usd)
