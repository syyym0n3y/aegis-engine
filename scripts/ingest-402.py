#!/usr/bin/env python3
# ingest-402.py (D-506) — 8-K Item 4.02 events via EDGAR full-text search, monthly windows, sequential.
...
while d<=dt.date(2026,8,1):
    ...
    frm=0
    while True:
        q=urllib.parse.urlencode({"q":'"Item 4.02"',"forms":"8-K","dateRange":"custom",
        ...
        for h in hits:
            s=h["_source"]
            if "4.02" not in (s.get("items") or []): continue
...
sql=("create temp table stage_e (accession text, symbol text, filed date);\n"
     ...
     "insert into trd_events_402 select * from stage_e on conflict (accession) do nothing;\n")
...
print(f"TOTAL {len(uniq):,} 4.02 events")
