...
async function cycle(n: number) {
  ...
  const N = 1000, ceil = Math.sqrt(2 * Math.log(N));
  ...
  await fetch(`${REST}/trd_autopilot_log`, { method: "POST", headers: { ...hdr, Prefer: "return=minimal" }, body: JSON.stringify({
    cycle_n: n, data_rows: rows.length, best_factor: best.factor, best_psr_z: best.psr_z, noise_ceiling: +ceil.toFixed(2),
    n_clearing, position_score, delta_vs_prev: delta, armed: ARMED, surfaced, verdict,
  }) });
  console.log(`[cycle ${n}] rows=${rows.length} best=${best.factor}(z${best.psr_z}) ceil=${ceil.toFixed(2)} clearing=${n_clearing} score=${position_score} delta=${delta ?? "—"}${surfaced ? " | SURFACED" : ""}`);
}
