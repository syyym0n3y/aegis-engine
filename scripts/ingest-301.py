#!/usr/bin/env python3
# ingest-402.py (D-516) — 8-K Item 3.01 events via EDGAR full-text search, monthly windows, sequential.
...
while d<=dt.date(2026,8,1):
    ...
    frm=0
    while True:
        q=urllib.parse.urlencode({"q":'"Item 3.01"',"forms":"8-K","dateRange":"custom",
        ...
        for h in hits:
            s=h["_source"]
            if "3.01" not in (s.get("items") or []): continue
...
sql=("create temp table stage_e (accession text, symbol text, filed date);\n"
     ...
     "insert into trd_events_301 select * from stage_e on conflict (accession) do nothing;\n")
...
print(f"TOTAL {len(uniq):,} 3.01 events")
