...
const CLASSES: Cls[] = [
  ...
  { name: "Equity indices / ETFs", universeQ: "trd_bars_deep?asset_class=in.(etf,index,intl_index,sector)&select=symbol",
    ...
    tested: "vol-timing, macro-regime conditioning, breadth U-shape, trend de-risk (ladder)",
  ...
  { name: "FX (majors)", universeQ: "trd_bars_deep?asset_class=eq.fx&select=symbol",
    ...
    odds: "NO SIGNIFICANT CARRY (RUN, D-738). G6 rate-differential long-short: Sharpe 0.22, t 1.06 over 272 months, and the directional prior is MISSED (pooled slope wrong-signed) — high-carry developed currencies depreciated on net. Consistent with the post-GFC developed-carry collapse; the EM high-yielders where the textbook premium lives are outside the placeable majors. Currency-of-account is a real STRUCTURAL effect (~1.3pp/yr for a GBP investor, D-731) but path-dependent, not a signal.",
    conf: "near-zero to time FX; carry measured absent in G6 (D-738); breadth 6 makes even that UNTESTED as a cross-section",
    risk: "leverage-driven blowup risk; central-bank regime shifts; 17.7y underwater on the carry book",
    gap: "EM high-yield currencies (where carry concentrates) not held; foreign LONG rates still partial" },
  { name: "Commodities", universeQ: "trd_bars_deep?asset_class=eq.commodity&select=symbol",
    ...
    odds: "CARRY/ROLL UNTESTED (no 2nd contract). Seasonality + ratios are CONTEXT drivers, not standalone edges.",
    conf: "near-zero; the defining commodity edge (roll yield) is data-blocked",
    risk: "storage/delivery, contango bleed, geopolitical shocks",
    gap: "2nd futures contract (roll yield), EIA/USDA inventories (free, unfetched)" },
