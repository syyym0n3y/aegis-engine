...
const outName = ((CROWD > 0 && K.DEPLOY_CROWD !== "1") || CAP > 0) ? K.ALT_OUT : "d931-gcshort-daily.json";  // D-971: adoption flag lifts the D-946 safety for CROWD_DC only
await Deno.writeTextFile(new URL(`../data/${outName}`, import.meta.url), JSON.stringify(toWrite));
console.log(`  wrote ${K.WRITE} series -> data/${outName}${K.WRITE === "gconly" ? "  (ISOLATION RUN — re-run with WRITE=combined to restore the deployable sleeve)"  : CROWD > 0 ? (K.DEPLOY_CROWD === "1" ? "  (D-971 ADOPTED: deployed d931 carries the CROWD_DC filter)" : "  (CROWD_DC MEASUREMENT — d931 untouched)") : ""}`);
if (CROWD > 0) {
