...
// mtf-volume-broad.ts (D-767) — the operator's "volume conditions" deep dive. The reversion signs found by
...
const K = declareKnobs("mtf-volume-broad", [
...
async function jwt() {
  ...
  const h = e({ alg: "HS256", typ: "JWT" }), b = e({ role: "service_role", iss: "mvb", exp: 4102444800 });
...
const spend = await spendTrials({ rest: OWNED, headers: hdr, family: "mtf-volume-broad", runId: `mvc|${K.SPLIT}`, spent: TRIALS });
...
if (K.DUMP) {
  ...
  await Deno.writeTextFile(K.DUMP, JSON.stringify({ source: "mtf-volume-broad.ts", written: new Date().toISOString(), split: K.SPLIT, cells }));
