...
// mtf-symmetric-side.ts (D-771) — the symmetry test the program had not yet run. D-768 measured the DOWNSIDE
...
const K = declareKnobs("mtf-symmetric-side", [
...
async function jwt() {
  ...
  const h = e({ alg: "HS256", typ: "JWT" }), b = e({ role: "service_role", iss: "mss", exp: 4102444800 });
...
const spend = await spendTrials({ rest: OWNED, headers: hdr, family: "mtf-symmetric-side", runId: `mss|${K.SPLIT}`, spent: TRIALS });
console.log(`\n  ================================ VERDICT ================================`);
