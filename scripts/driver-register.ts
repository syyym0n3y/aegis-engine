...
const REG: Driver[] = [
  ...
  { cls: "gold", name: "REAL yields (TIPS)", use: "condition",
    probe: "trd_macro_series?series=eq.real_yield_10y&select=d&limit=1",
    proxy: "DFII5/10/30 from FRED's KEYLESS fredgraph.csv (D-729) — the 'FRED credential-gated' label was an edge-environment fact, not a data fact" },
  { cls: "gold", name: "CPI / inflation expectations", use: "condition",
    probe: "trd_macro_series?series=eq.breakeven_10y&select=d&limit=1",
    proxy: "CPI, core CPI and 5y/10y breakevens (D-729, FRED keyless); survey expectations still not held" },
  ...
  { cls: "gold", name: "ETF flows (GLD/IAU shares outstanding)", use: "condition",
    probe: "trd_macro_series?series=eq.gld_tonnes&select=d&limit=1",
    proxy: "GLD daily tonnes + shares outstanding from the issuer's archive (D-745), live in the daily runner; IAU not yet ingested (GLD is ~2/3 of US gold-ETF holdings)" },
  ...
  { cls: "equity-index", name: "breadth (adv/decl, %>200dma, new-high/low)", use: "condition",
    probe: "trd_macro_series?series=eq.breadth_pct_gt_200dma_surv&select=d&limit=1",
    proxy: "computed in-DB from the panel (D-717); SURVIVORSHIP-BIASED level — dead names absent, so the moves are the signal, not the level" },
  { cls: "equity-index", name: "options regime (SKEW / VVIX / VIX3M term)", use: "condition",
  ...
  { cls: "equity-index", name: "dealer gamma HISTORY (SqueezeMetrics GEX + DIX, 2011->)", use: "condition",
    probe: "trd_macro_series?series=eq.sqm_gex&select=d&limit=1",
    proxy: "HELD (D-818): free keyless daily SPX GEX ($) + dark-pool index since 2011-05 (ingest-squeezemetrics.ts, series sqm_gex/sqm_dix/sqm_spx). Tested NULL: no vol information beyond VIX3M (0/8 years), no return effect (GEX / DIX)." },
  { cls: "equity-index", name: "estimate revisions, Nasdaq cross-source snapshot", use: "condition",
    probe: "trd_macro_series?series=like.nq_revup_*&select=d&limit=1",
    proxy: "HELD FORWARD (D-818): Nasdaq analyst endpoint (keyless, browser UA) — consensus EPS, estimate count, 4-week up/down revision counts for the 150 most liquid names daily (collect-nasdaq-revisions.ts). Second source beside the Yahoo earningsTrend series; no free history exists." },
  { cls: "equity-single", name: "options surface per name (wide daily snapshot: P/C OI, ATM IV, skew, naive GEX)", use: "condition",
    probe: "trd_perp_oi?venue=eq.cboe&interval=eq.names_collected&symbol=eq._WIDE&select=ts&limit=1",
    proxy: "HELD forward-only (D-817): CBOE delayed chains answer for any underlying with per-strike OI and IV; collect-us-options.ts WIDE=1 snapshots the liquid decile daily. NO free keyless per-strike HISTORY exists (probed: OCC = market totals; Massive = key + current OI; OptionsDX = checkout; OptionCharts = no endpoint; HistoricalData.net = 2013 sample) — history is Theta ($40/mo) or the accumulating snapshot." },
  { cls: "equity-index", name: "per-strike SPX open interest HISTORY (5 months)", use: "condition",
    probe: "trd_macro_series?series=eq.databento_spxw_oi_days&select=d&limit=1",
    proxy: "HELD (D-820): SPXW per-strike daily OI 2026-03-16..2026-09-03 (120 expiries) from Databento OPRA statistics on the operator's credit ($26.06; ~$3.40 remains). Tested: pinning SIGN MISSED (the close leaves the max-OI strike), walls underpowered. Extending history is metered (Theta $40/mo for 4 years)." },
  ...
  { cls: "equity-index", name: "earnings revisions", use: "condition",
    probe: "trd_macro_series?series=eq.est_rev90_0q:AAPL&select=d&limit=1",
    proxy: "HELD (D-817): keyless — Yahoo earningsTrend gives consensus EPS now vs 7/30/60/90 days ago plus up/down revision counts per name; Nasdaq analyst endpoint is the cross-source control (AAPL within 5%). Daily snapshots for the liquid decile in the runner (est_rev90_0q/0y, est_up30, est_down30, est_n_0q). Forward test D-817-estimate-revisions-forward at >= 12 monthly snapshots." },
  // ---- FX ------------------------------------------------------------------------------------------------------
  ...
  { cls: "fx", name: "FOREIGN short rates (the carry leg)", use: "condition",
    probe: "trd_macro_series?series=eq.rate_3m_jpy&select=d&limit=1",
    proxy: "14 currencies from FRED keyless (D-737/741): 3m interbank for G6+NZD+MXN/ZAR/KRW/CNY, overnight/policy for BRL/INR/TRY. Carry TESTED: G6 null (D-738), EM a research-space risk premium (D-741)." },
  // ---- COMMODITY -----------------------------------------------------------------------------------------------
  ...
  { cls: "commodity", name: "FUTURES CURVE (contango/backwardation)", use: "condition",
    probe: "trd_macro_series?series=eq.eia_cl_c2&select=d&limit=1",
    proxy: "EIA keyless XLS contracts 1-4 for CL (1983+) and NG (1994+), ending 2024-04 — a research history, not live (D-742). GC/ZC/ZW/ZS/HG/SI: no free historical curve found. Roll yield TESTED: CL null OOS, NG ruined." },
  { cls: "commodity", name: "inventories / storage", use: "explain",
    probe: "trd_macro_series?series=eq.eia_crude_stocks_w&select=d&limit=1",
    proxy: "EIA weekly commercial crude ex-SPR + Lower-48 gas storage, keyless and LIVE in the daily runner (D-743); post-release change conditioning NULL. USDA ag inventories still unfetched." },
  // ---- CRYPTO --------------------------------------------------------------------------------------------------
  { cls: "crypto", name: "price, hourly + daily SF panel", use: "predict", probe: "trd_bars_intraday?tf=eq.1dSF&select=symbol&limit=1" },
  { cls: "crypto", name: "implied-vol index HISTORY (Deribit DVOL BTC/ETH, 2021->)", use: "condition",
    probe: "trd_macro_series?series=eq.deribit_btc_dvol&select=d&limit=1",
    proxy: "HELD (D-818): Deribit DVOL daily since 2021-03 (ingest-deribit-dvol.ts). Tested: BTC variance premium CONFIRMED as a measurement (NW t 4.39, 74% months positive), DVOL a strong BTC 30d vol-forecast input; ETH fails; next-day return effect 1.5-1.6x fee at t 1.4-2.1, not claimable." },
  { cls: "crypto", name: "volatility forecast (HAR-RV, next day) — sizing input", use: "condition",
    probe: "trd_macro_series?series=eq.harrv_vol_1d:BTCUSDT&select=d&limit=1",
    proxy: "HELD (D-819): daily HAR-RV forecast per instrument (3 perps + 8 FX/index), harrv-daily.ts in the runner; the model class confirmed OOS on 11/11 (D-814). Variance, not direction." },
  ...
  { cls: "equity-single", name: "order flow / depth (Nasdaq TotalView TBBO)", use: "predict",
    probe: "trd_macro_series?series=eq.databento_tbbo_days_TSLA&select=d&limit=1",
    proxy: "HELD (D-816): trades-with-book for the 20 most liquid names, 2026-04..2026-09 (Databento, $95.52 of the operator's $125 credit; files data/databento). Tested: top-of-book imbalance, aggressor delta, footprint proxy all SUB-FEE (0/20 names clear 1x fee). Extending the span or universe is metered spend." },
