...
async function orderFlow() {
  ...
  for (const s of ["SPY", "_SPX"]) {
    ...
    if (g?.ok) gex.push({ symbol: g.symbol, spot: g.spot, regime: g.regime, regimeRead: g.regime_read, callWall: g.call_wall, putWall: g.put_wall, flip: g.gamma_flip, totalGexBn: g.total_gex_bn });
  }
  const va: any[] = [];
...
async function gather() {
  ...
  return { st, a, equity, start, mult, dd, ticks: st?.ticks ?? 0, updated: st?.updated_at ?? null, openPos: a?.positions?.length ?? 0, closed: a?.closed?.length ?? 0, survived: equity > start * 0.5, perSess, grades, reports: (reps as any[]).length, macro, hyps, corpusN: (corpus as any[]).length, rejected, cyclePrediction, volRegime, fresh, liveRisk, meanrev, gex: orderFlowLevels.gex, valueAreas: orderFlowLevels.va };
}
function j(d: any) { return { protocol_version: 1, project_kind: "aegis", generated_at: new Date().toISOString(), summary: { status: d.survived ? "alive" : "breached", equity: Math.round(d.equity), multiple: +d.mult.toFixed(3), max_drawdown_pct: +(d.dd * 100).toFixed(1), ticks: d.ticks, open_positions: d.openPos, closed_trades: d.closed, platform_reports: d.reports, macro_phase: d.macro?.phase ?? "UNKNOWN", macro_de_risk: d.macro?.deRisk ?? 1 }, data: { per_session_setup: d.perSess, grades: d.grades, factor_book_sharpe: 1.0, macro: d.macro, vol_regime: d.volRegime, tracker_freshness: d.fresh, live_account_risk: d.liveRisk?.snapshot ?? null, meanrev_scanner: d.meanrev, prereg_hypotheses: d.hyps, corpus_assessed: d.corpusN, corpus_rejected: d.rejected, cycle_prediction: d.cyclePrediction, gex_levels: d.gex, value_areas: d.valueAreas }, open_work: ["live real-money bridge (paper-first, gated)", "close risk-inventory gaps", "auth/billing", "branded domain"], recent_events: [], query_errors: [] }; }
...
${gexArr.length ? `<table><thead><tr><th>underlying</th><th class=num>spot</th><th>gamma regime</th><th class=num>put wall (support)</th><th class=num>gamma flip</th><th class=num>call wall (resist)</th></tr></thead><tbody>${gexArr.map((g: any) => `<tr><td class=mono>${esc(g.symbol)}</td><td class="mono num">${g.spot}</td><td class=mono style="color:${regCol(g.regime)}">${esc(g.regime)} ${g.totalGexBn >= 0 ? "+" : ""}${g.totalGexBn}B</td><td class="mono num" style="color:var(--accent)">${g.putWall}</td><td class="mono num" style="color:${near(g.spot, g.flip)}">${g.flip ?? "—"}</td><td class="mono num" style="color:var(--red)">${g.callWall}</td></tr>`).join("")}</tbody></table>` : ""}
