...
Deno.serve(async()=>{const cors={"Content-Type":"application/json","Access-Control-Allow-Origin":"*"};try{
  const [arm,ks,pairsOpen,snapArr]=await Promise.all([
  ...
  const attribute=(sym:string,asset:string):string=>{
    if(asset==="crypto")return "crypto-momentum";
    if(sym==="SVXY")return "vrp";
    if(pairLegs.has(sym))return "pairs";
    return "rip-short";                                   // rip-short is the only equity-short edge
  };
  const posOut=(Array.isArray(positions)?positions:[]).map((p:{symbol:string,side:string,qty:string,avg_entry_price:string,current_price:string,unrealized_pl:string,unrealized_plpc:string,asset_class?:string})=>({
    sym:p.symbol,edge:attribute(p.symbol,p.asset_class||"us_equity"),side:p.side,qty:+p.qty,
  ...
  const pend=(Array.isArray(openOrders)?openOrders:[]).map((o:{symbol:string,side:string,qty:string,asset_class?:string})=>({sym:o.symbol.replace("/",""),edge:attribute(o.symbol.replace("/",""),o.asset_class||"us_equity"),side:o.side,qty:+o.qty}));
