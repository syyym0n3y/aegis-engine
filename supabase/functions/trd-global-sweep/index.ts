...
// meanR(random) in currency-NEUTRAL R units (no FX needed). Verdict per name. Idempotent: swept rows excluded next
// tick. Cron grinds ~46k names over many ticks. NO order path — pure measurement.
...
async function bars(sym:string):Promise<{b:Bar[],cur:string}|null|"throttle">{try{
  ...
  if(r.status===429||r.status>=500)return "throttle";      // don't mark swept — retry next tick
...
// short capped-stop outcome in R from entry day i
...
Deno.serve(async(req)=>{const cors={"Content-Type":"application/json","Access-Control-Allow-Origin":"*"};try{
  const N=+(new URL(req.url).searchParams.get("limit")||"60");
  ...
  for(const row of batch as {yahoo_sym:string,ticker:string,exchange:string}[]){
    const sym=row.yahoo_sym;const data=await bars(sym);
    if(data==="throttle"){if(++throttled>=3)break;continue;}   // Yahoo backing off — leave unswept, stop after 3
    // every row MUST carry the identical key set (PostgREST bulk-upsert requires it); fill nulls for the rest
