...
| a loop restart DUPLICATED a running step (2026-09-07 20:47Z: the wide options snapshot started by the runner while another session's instance of it was mid-run; two streams at CBOE, a 429 source, until the peer killed its copy) | `assertSingleInstance()` in `run-preconditions.ts` (D-818): a collector refuses to start when another instance of itself is running; applied to the Nasdaq revisions and Binance mirror collectors; offered to the peer for the options collector. Rule: with two sessions on the repo, no kickstart while a peer's long step is running |
