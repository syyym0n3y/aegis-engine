...
7. **Residency and destination restrictions** (D-823c → D-823d, read from FTMO's "Who can join" page 2026-09-08): the restriction is on the trader's NATIONALITY/RESIDENCY (Iran, Syria, Myanmar, North Korea absolutely; a long business-risk list incl. Russia, Belarus, Indonesia, Kazakhstan); **the UK is not on it**; the page states no restriction on the payout destination. Account currencies: the withdrawal page states rollover minimums in €/$/£/AUD/CAD/CHF/CZK, which implies GBP- and USD-denominated accounts exist and rewards are processed in the account currency — LIKELY, not stated in one sentence; confirm at checkout. Open: whether a reward from a GBP account can be sent to a non-UK USD bank without a conversion at FTMO's rate (crypto USDC/USDT payout is the currency-agnostic method on the page).
