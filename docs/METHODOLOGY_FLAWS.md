...
| C2 | **The UTC day boundary is an arbitrary choice that the results may depend on** | D-828 found crypto extremes cluster 00:00–06:00 UTC. A "day" for a 24-hour instrument is a convention, and every prior-session level, daily range and day-clustered statistic inherits it | recompute the D-829 range statistics with the boundary shifted 6, 12 and 18 hours; a real effect survives, an artifact moves | every prior-day/prior-session level rule on crypto — a large share of D-763/764/767/827/829 |
...
| C4 | **TESTED SAME DAY — CONFIRMED, 5 of 30 exposed.** Guards are verified once, at birth, and never re-verified | four guards have been found broken AFTER shipping (D-584 fail-open, D-650, D-659 silent, D-671). `registry-guard` checks a guard EXISTS; nothing checks it can still go RED | **DONE: `scripts/guard-selftest-all.ts`, in the daily runner.** Result: **25 of 27 demonstrate a working red branch; 2 do not** (`coverage-guard` — its "self-test" is a STRICT mode that never exercises a synthetic subject; `market-cap-guard` — exits 0 without declaring a pass, the D-659 silent shape), and **3 have no self-test at all** (`continuity-guard`, `gap-register-guard`, `infra-guard`) | the board's green, which is the single most load-bearing claim in the programme. **5 of 30 guards cannot currently be shown able to refuse anything** |
